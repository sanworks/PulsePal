...
//
// This is a copy of /Firmware/PulsePal3/ArCOM.h for Wave Pal, which uses it for its own protocol (PROTOCOL.md in this
// folder). The only change is the comment on ARCOM_WRITE_BUFFER_SIZE. Keep the two copies in step.
//
// How it behaves:
//
...
#define ARCOM_WRITE_BUFFER_SIZE 256 // Must hold the largest reply: 178 bytes in Pulse Pal firmware (ops 85 and 93),
                                    // 37 bytes in Wave Pal firmware (op 71)
