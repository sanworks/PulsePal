...
byte CustomTrainID[4] = {0}; // If 0, uses above params. If 1 or 2, pulse times and voltages are played back from CustomTrain1 or 2
byte CustomTrainTarget[4] = {0}; // If 0, custom times define start-times of pulses. If 1, custom times are start-times of bursts.
byte CustomTrainLoop[4] = {0}; // if 0, custom stim plays once. If 1, custom stim loops until PulseTrainDuration.
...
unsigned long SSdelay = 1200000; // Idle cycles until screen saver is activated (1 minute at 20kHz)
...
//        } break;
        case 89: { // Receive new CommanderString (displayed on top line of OLED, i.e. "MATLAB connected"
          PPUSB.readCharArray(CommanderString, 6);
          for (int x = 6; x < 16; x++) {
                 ...
                 if (CustomTrainID[x] == 0) {
                   ...
                   if (CustomTrainID[x] == 1) {
                     if (CustomVoltages[0][CustomPulseTimeIndex[x]] < 32768) {
                       dacValue.uint16[x] = 32768 + (32768 - CustomVoltages[0][CustomPulseTimeIndex[x]]); DACFlag = 1;
                     } else {
                       dacValue.uint16[x] = 32768 - (CustomVoltages[0][CustomPulseTimeIndex[x]] - 32768); DACFlag = 1;
                     }
                   } else {
                     if (CustomVoltages[1][CustomPulseTimeIndex[x]] < 32768) {
                       dacValue.uint16[x] = 32768 + (32768 - CustomVoltages[1][CustomPulseTimeIndex[x]]); DACFlag = 1;
                     } else {
                       dacValue.uint16[x] = 32768 - (CustomVoltages[1][CustomPulseTimeIndex[x]]-32768); DACFlag = 1;
                     } 
                   }
                   if (CustomTrainTarget[x] == 0) {
                       CustomPulseTimeIndex[x] = CustomPulseTimeIndex[x] + 1;
        ...
        if (SystemTime == NextBurstTransitionTime[x]) {
          ...
          // Determine if burst status should go to 1 now
            ...
            if ((CustomTrainID[x] > 0) && (CustomTrainTarget[x] == 1)) {              
              if (CustomTrainID[x] == 1) {
                 if (CustomPulseTimeIndex[x] < CustomTrainNpulses[0]){
                    dacValue.uint16[x] = CustomVoltages[0][CustomPulseTimeIndex[x]]; DACFlag = 1;
                 }
              } else {
                if (CustomPulseTimeIndex[x] < CustomTrainNpulses[1]){
                    dacValue.uint16[x] = CustomVoltages[1][CustomPulseTimeIndex[x]]; DACFlag = 1;
                 }
              }
            } else {
                 dacValue.uint16[x] = Phase1Voltage[x]; DACFlag = 1;
...
void LoadDefaultParameters() {
  ...
  for (int x = 0; x < 4; x++) {
      Phase1Duration[x] = 20;
      InterPhaseInterval[x] = 20;
      Phase2Duration[x] = 20;
      InterPulseInterval[x] = 200;
      BurstDuration[x] = 0;
      BurstInterval[x] = 0;
    ...
    for (int y = 0; y < 4; y++) {
      TriggerAddress[0][y] = 0;
    }
    for (int y = 0; y < 4; y++) {
      TriggerAddress[1][y] = 0;
    }
   TriggerAddress[0][0] = 1;
   TriggerMode[0] = 0; 
   TriggerMode[1] = 0;
